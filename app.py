// =============================================================================
// سكريبت المنصة الرقمية المعتمد - الإصدار المصحح: عزل البند 39 وحصره تشغيلياً
// =============================================================================
...
function doPost(e) {
  try {
    ...
    // 1. توليد تقرير المركز المستقل
    var centerFileUrl = createCustomCenterReport(data);

    // 2. الأرشفة المركزية بتوقيت الرياض الصريح
    ...
    // استبعاد البند 39 من نقاط الضعف في الأرشيف
    var weakSummaryList = [];
    if (data.responses && data.responses.length > 0) {
      for (var i = 0; i < data.responses.length; i++) {
        var rItem = data.responses[i];
        var itemId = parseInt(rItem.id);
        if (itemId === 39) continue; // استبعاد البند 39 تماماً
    ...
    var safeMatchedCnt = Math.min(parseInt(data.matched_cnt) || 0, 38);
...
function createCustomCenterReport(data) {
  ...
  // حصر الأعداد في نطاق الـ 38 معياراً فقط
  var matchedCnt = Math.min(parseInt(data.matched_cnt) || 0, 38);
  var partialCnt = parseInt(data.partial_cnt) || 0;
  var unmatchedCnt = parseInt(data.unmatched_cnt) || 0;
  ...
  // استخراج الملاحظات مع استبعاد البند 39 تماماً من جداول الضعف والخطة
  var weakItems = [];
  if (data.responses && data.responses.length > 0) {
    for (var i = 0; i < data.responses.length; i++) {
      var item = data.responses[i];
      var itemId = parseInt(item.id);
      if (itemId === 39) continue; // تخطي البند 39 وعدم إدراجه في الخطة أو الضعف
  ...
  if (weakItems.length > 0) {
    ...
    // حالة الامتثال التام 100% للمحاور الـ 38
      ...
      for (var k = 0; k < values.length; k++) {
        ...
        if (!isNaN(num) && respMap[num]) {
          ...
          if (rawStatus === "مطابق") {
            ...
            matchPct = "0.0%";
          } else if (rawStatus === "يوجد" || rawStatus === "لا يوجد") {
            matchStatus = rawStatus;
            matchPct = "-";
          }
...
function sendApprovedEmailReport(data, centerFileUrl) {
  ...
  if (data.responses && data.responses.length > 0) {
    for (var i = 0; i < data.responses.length; i++) {
      var item = data.responses[i];
      var itemId = parseInt(item.id) || (i + 1);
      if (itemId === 39) continue; // استبعاد البند 39 من قائمة نقاط التدقيق في البريد
  ...
  var bodyText =
    ...
    "ملخص النتائج:\n" +
    "• عدد البنود المطابقة: " + Math.min(parseInt(data.matched_cnt) || 0, 38) + " من أصل 38\n" +
